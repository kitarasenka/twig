...
--follow`). Клик по строке переводит граф на этот коммит через тот же `jump`,
что и клик по родителю. Argv строит `buildFileHistoryArgv` в `main/git/history.js`
(single pathspec как `:(literal)<path>` после `--`), выполняет `loadFileHistory`
через тот же `runGit`; парсер `parseHistoryV1` переиспользован без изменений.
...
`history-smoke.mjs` открывает меню, сверяет список из двух коммитов и переход по
клику, плюс два отказа IPC на traversal/абсолютном пути.
...
`foundation.mjs` дополнена кейсами `mailto:`. Версия рабочего дерева 0.3.0.
...
заметка выбираются в панели коммита (свотчи + textarea + «Save note»); пункт
меню только открывает панель на нужном коммите. Помеченная строка в графе
получает фоновую подсветку `color-mix`, полосу у правого края, кольцо вокруг
узла и иконку-закладку с заметкой в тултипе — поверх любого режима «Commit
colors» (age/lanes). Цвет — не единственный носитель смысла: есть иконка и
`aria-label`. Хранится в `marks.json` в `userData`, ключ — id репозитория (это
...
- `main/git/commit-ops.js` — `createCommit`, стеш и `rewordHead`. Reword сверяет
  HEAD с oid, который показывал renderer, и отказывается, если ветка успела
  уехать: сообщение написано для одного коммита, а `--amend` переписал бы тот,
  которым HEAD стал.
