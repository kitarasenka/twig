...
**Current version: 0.1.0 — M1.** Twig detects system Git, opens local repositories,
shows the current branch and working-tree file count, and records every Git command
with its output. The sample history remains fictional until M2; clone and mutations
are still disabled.
...
```sh
npm test          # ESLint + parser/executor checks; uses a temporary Git repository
...
ignored `artifacts/`. Its temporary profile only runs the startup Git check.

## Next milestones and packaging

M2 replaces sample history with paginated,
...
preload exposes only explicit app, repository and console APIs, with
sender/frame/argument validation in main. Its ESM source is bundled into a
sandbox-compatible CommonJS artifact.
